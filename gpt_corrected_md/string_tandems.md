...
**Тандемним повтором** (тандемним повторенням) називаються два входження однієї і тієї ж підрядки, що знаходяться в рядку поруч. Іншими словами, тандемне повторення можна описати парою індексів $i < j$, для яких підрядок $s[i \ldots j]$ складається з двох однакових підряд розташованих стрічок.
...
Розглянемо тандемні повтори на прикладі якої-небудь простої рядка, наприклад:
...
Загалом кажучи, кількість тандемних повторів у рядокх довжини $n$ може бути порядку $O(n^2)$.

Очевидним прикладом є рядок, складена з $n$ однакових букв - у такій рядку тандемним повтором є будь-який підрядок парної довжини, кількість яких становить приблизно $n^2 / 4$. Загалом, будь-яка періодично рядок з коротким періодом буде містити дуже багато тандемних повторів
...
Більше того, існує поняття **серій** - четвірок чисел, які описують цілу групу періодичних підрядків. Було доведено, що кількість серій у будь-якій рядку лінійно залежить від довжини рядка.
...
* Відомо, що якщо розглядати лише примітивні тандемні повтори (тобто ті, чиї половинки не є кратними рядками), то їх кількість у будь-якій рядку складає $O(n \log n)$.

* Якщо кодувати тандемні повтори трійками чисел $(i,p,r)$ (називається трійками Крочемора (Crochemore)), де $i$ - позиція початку, $p$ - довжина повторюваної підрядки, $r$ - кількість повторів, то всі тандемні повтори будь-якої рядка можна вивести з допомогою $O(n \log n)$ таких трійок. Такий результат отримується на виході алгоритму Крочемора знаходження всіх тандемних повторів.
...
Число тандемних повторів у $i$-ій рядку Фібоначчі довжини $f_i$, навіть стиснених за допомогою трійок Крочемора, становить $O(f_i \log f_i)$.
...
Отже, алгоритм Мейн-Лоренца зводиться до того, щоб навчитися шукати всі перетинаючіся тандемні повтори за заданою рядком $s$, тобто такі, що починаються в першій половині рядка і закінчуються в другій.

Позначимо $u$ та $v$ дві підрядки рядка $s$, які є її половинками
...
Розглянемо довільний тандемний повтор і подивимося на його середній символ (точніше, на той символ, з якого починається друга половина тандема; тобто якщо тандемний повтор - це підрядок $s[i \ldots j]$, то середнім символом буде $(i+j+1)/2$.

Значить, назвемо тандемний повтор "лівим" або "правим" залежно від того, де знаходиться цей символ - у рядку $u$ чи у рядку $v$. (Можна сказати і так: тандемний повтор називається "лівим", якщо більша його частина лежить у лівій половині рядка $s$; інакше - тандемний повтор називається "правим").
...
Позначимо довжину шуканого лівого тандемного повтору через $k$ (тобто довжина кожної половинки тандемного повтору - це $k$). Розглянемо перший символ тандемного повтору, що потрапляє в рядок $v$ (він має значення в рядку $s$ на позиції $length(u)$). Він збігається з символом, що стоїть на $k$ позицій раніше нього; позначимо цю позицію через $cntr$.
...
Отримуємо таку схему (для абстрактної рядка, в якій міститься тандемний повтор $"abcabc"$):
...
* Для швидкого знаходження значень $k_1$ заздалегідь порахуємо Z-функцію для рядка $\overline{u}$ (тобто рядка $u$, записаної в зворотному порядку).
...
Наведемо реалізацію алгоритму Мейн-Лоренца, яка за час $O(n \log n)$ знаходить всі тандемні повтори даної рядка у стислому вигляді (у вигляді груп, описуваних чотирма числами).
