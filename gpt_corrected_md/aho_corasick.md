...
Розглянемо в лісі будь-який шлях з кореня; випишемо послідовно мітки ребер цього шляху. В результаті ми отримаємо деяку рядок, яка відповідає цьому шляху. Якщо ж ми розглянемо будь-яку вершину префіксного дерева, то їй поставимо у відповідність рядок, відповідну шляху з кореня до цієї вершини.

Кожна вершина префіксного дерева також має прапорець $\rm leaf$, який дорівнює $\rm true$, якщо в цій вершині закінчується яка-небудь рядок з даного набору.
...
Більш строго, нехай ми знаходимося в стані $p$, якому відповідає деяко рядок $t$, і бажаємо виконати перехід по символу $c$. Якщо в борі з вершини $p$ є перехід за літерою $c$, то ми просто переходимо по цій дуги і потрапляємо в вершину, якій відповідає рядок $tc$. А якщо такої дуги немає, то ми повинні знайти стан, відповідний найдовшому власному суфіксу рядка $t$ (найдовшому з наявних в борі), і спробувати виконати перехід по літері $c$ з нього.

Наприклад, якщо префіксне дерево побудовано за рядками "аб" і "бц", а ми, під впливом рядка "аб", перейшли в деякий стан, який є листком, то це означає, що під впливом букви "ц" ми змушені перейти в стан, що відповідає рядку "б", і тільки звідти виконати перехід по букві "ц".
...
### Знаходження лексикографічно найменшої рядка заданої довжини, яка не містить жодного з заданих зразків

Даний набір зразків та довжина $L$. Потрібно знайти рядок довжини $L$, яка не містить жодного з зразків, та серед усіх таких стрічок вивести лексикографічно найменшу.

Побудуємо за даним набором рядків префіксне дерево. Нагадаємо, що ті вершини, з яких можна досягти помічених вершин за допомогою суфіксних посилань (а такі вершини можна знайти за $O(n)$, наприклад, лінивою динамікою), можна сприймати як входження якої-небудь рядка з набору в текст завдання. Оскільки в даній задачі ми повинні уникати входжень, то це можна розуміти як те, що в такі вершини ми не можемо заходити. З іншого боку, в усі інші вершини ми можемо заходити. Таким чином, ми видаляємо з автомата всі "погані" вершини, а з залишку графу автомата потрібно знайти лексикографічно найменший шлях довжини $L$. Цю задачу вже можна розв'язати за $O(L)$, наприклад, за допомогою [пошуку в глибину](dfs).

### Знаходження найкоротшої рядка, що містить усі зразки одночасно
...
### Знаходження лексикографічно найменшої рядка довжини $L$, що містить дані зразки у кількості $k$ раз
