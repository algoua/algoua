...
Нехай дано рядок $s$ довжини $n$. Значення **Z-функції** ("зет-функції") для цієї рядка - це масив довжини $n$, $i$-й елемент якого дорівнює найбільшій кількості символів, починаючи з позиції $i$, які збігаються з першими символами рядка $s$.
...
Назвемо для стислості підрядок, що збігається з префіксом рядка $s$, **відрізком збігу**. Наприклад, значення шуканої Z-функції $z[i]$ - це довгий відрізок збігу, що починається в позиції $i$ (і закінчується в позиції $i + z[i] - 1$).

Для цього будемо підтримувати **координати $[l;r]$ найправішого відрізку збігу**, тобто з усіх виявлених відрізків будемо зберігати той, який закінчується правіше за всіх. В деякому сенсі, індекс $r$ - це така межа, до якої нашо рядок вже була просканована алгоритмом, а все інше - поки що не відомо.
...
Для цього зауважимо, що підрядки $s[l \ldots r]$ і $s[0 \ldots r-l]$ **співпадають**. Це означає, що в якості початкового наближення для $z[i]$ можна взяти відповідне йому значення з рядка $s[0 \ldots r-l]$, а саме, значення $z[i-l]$.
...
Доведемо, що наведений вище алгоритм працює за лінійний час щодо довжини рядка, тобто за $O(n)$.
...
Для розв'язання цієї задачі утворимо рядок $s = p + \text{\#} + t$, тобто до зразків додаємо текст через символ-роздільник (який не зустрічається ніде в самому тексті).

Порахуймо для отриманої рядка Z-функцію. Отже, для будь-якого $i$ в діапазоні $[0; length(t)-1]$ за відповідними значеннями $z[i + length(p) + 1]$ можна зрозуміти, чи входить образець $p$ в текст $t$, починаючи з позиції $i$: якщо значення Z-функції дорівнює $length(p)$, то так, входить, інакше - ні.
...
Отже, нехай $k$ - поточна кількість різних підрядків рядка $s$, і ми додаємо в кінець символ $c$. Очевидно, в результаті можуть з'явитися деякі нові підрядки, які закінчуються на цьому новому символі $c$ (а саме, всі підрядки, які закінчуються на цей символ, але не зустрічалися раніше).
...
Отже, ми отримали, що кількість нових підрядків, які з'являються при додаванні символу $c$, дорівнює $len - z_{\rm max}$, де $len$ - поточна довжина рядка після додавання символу $c$.

Отже, асимптотика розв'язку для рядка довжини $n$ становить $O(n^2)$.

Варто зазначити, що можна повністю аналогічно перераховувати кількість різних підрядків за $O(n)$ при додаванні символу в початок, а також при видаленні символу з кінця або з початку. Російське слово "повинен" слід замінити на "має".
