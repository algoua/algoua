...
При цим алгоритм базується на двох простих эквивалентных преобразованиях системи: у-перших, можна обменивать два рівняння, а у-вторых, будь-яке рівняння можна замінити лінійної комбинацией цій рядка (з ненульовим коефіцієнтом) і інших стрічок (з произвольными коефіцієнтами).

**На першому кроці** алгоритм Гауса-Жордана ділить першу рядок на коефіцієнт $a_{11}$. Потім алгоритм додає першу рядок до іншим рядках з такими коефіцієнтами, аби їх коефіцієнти в першому стовпці обращались в нулі - для цього, очевидно, при прибавлении першо] рядка до $i$-ой треба домножувати її на $-a_{i1}$. При кожної операції з матрицею $A$ (поділ на число, додаток до однієї рядка іншої) відповідні операції виробляються і з вектором $b$; в деякому сенсі, він веде собі, як якщо б він був $m+1$-им столбцом матриці $A$.

В результаті, по закінченні першого шага перший стовпець матриці $A$ стане единичным (тобто буде містити одиницю в першо] рядка і нулі в інших).

Аналогічно виготовляється другий крок алгоритму, тільки тепер розглядається другий стовпець і друго рядок: спочатку друго рядок ділиться на $a_{22}$, а потім віднімається від всіх інших стрічок з такими коефіцієнтами, аби обнулять другий стовпець матриці $A$.

І так далі, поки ми не опрацюємо всі рядка або всі стовпці матриці $A$. Якщо $n=m$, то по побудови алгоритму очевидно, що матриця $A$ вийде одиничної, що нам і потрібно.

### Пошук опорного елементу (pivoting)

Зрозуміло, описана вище схема неполна. Вона працює тільки в тому випадку, якщо на кожному $i$-ом кроці елемент $a_{ii}$ відмінний від нуля - інакше ми просто не зможемо домогтися обнуления інших коефіцієнтів в поточному стовпці шляхом додавання до ним $i$-ой рядка.
...
До счастью, для коректності методу достатньо одних тільки обмінів стрічок (т.н. "partial pivoting", в відміну від "full pivoting", коли обмениваются і рядка, і стовпці). Але яку ж саме рядок випливає вибирати для обмена? І правда або, що пошук опорного елементу треба робити тільки тоді, коли поточний елемент $a_{ii}$ нульовий?
...
Іншими словами, перед виконанням $i$-ой фази алгоритму Гауса-Жордана з евристикою partial pivoting необхідно знайти в $i$-ом стовпці серед елементів з індексами від $i$ до $n$ максимальний за модулем, і обміняти рядок з цим елементом з $i$-ой рядком.
...
Розглянемо тепер **загальний випадок** - коли $n$ і $m$ не обов'язково рівні. Припустимо, що опорний елемент на $i$-ом кроці не знайшовся. Це означає, що в $i$-ом стовпці всі рядка, починаючи з поточній, містять нулі. Стверджується, що в цим випадку ця $i$-ая змінна не можливо бути визначена, і є **независимой змінній** (можливо приймати довільне значення). Щоб алгоритм Гауса-Жордана продолжил свою роботу для всіх подальших змінних, в такий ситуації треба просто пропустити поточний $i$-ый стовпець, не увеличивая при цим номер поточній рядка (можна сказати, що ми виртуально видаляємо $i$-ый стовпець матриці).
...
В функції підтримуються два покажчика - на поточний стовпець $\rm col$ і поточну рядок $\rm row$.

Також заводиться вектор $\rm where$, в якому для кожної змінній записано, в який рядка повинна вона вийти (іншими словами, для кожного стовпчика записан номер рядка, в якій цей стовпець відмінний від нуля). Цей вектор потрібен, оскільки деякі змінні могли не "определиться" в під час розв'язку (тобто це незалежні змінні, яким можна присвоїти довільне значення - наприклад, в наведеної реалізації це нулі).

Реалізація використовує технику partial pivoting, производя пошук рядка з максимальним за модулем елементом, і переставляя потім цю рядок в позицію $\rm row$ (хоча явну перестановку стрічок можна замінити обменом двох індексів в деякому масиві, на практиці це не дасть реального выигрыша, т.до. на обмены тратится $O(n^2)$ операцій).

В реалізації в цілях простоти поточно рядок не ділиться на опорний елемент - так що в підсумку по закінченні роботи алгоритму матриця стає не одиничної, а диагональной (втім, мабуть, поділ рядка на ведущий елемент дозваляє декілька зменшити виникають похибки).
...
Евристика implicit pivoting полягає в тому, що елементи різних стрічок сравниваются так, як якщо б обидві рядка були пронормированы таким чином, що максимальний за модулем елемент в них був б рівний одиниці. Для реалізації цій техніки треба просто підтримувати поточний максимум в кожної рядка (або підтримувати кожну рядок так, аби максимум в ній був рівний одиниці за модулем, але це можливо привести до збільшення накапливаемой похибки).
