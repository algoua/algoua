...
Нехай дано рядок $s$ довжини $n$. Значить **Z-функція** ("зет-функція") від цій рядка - це масив довжини $n$, $i$-ый елемент якого рівний найбільшому числу символів, починаючи з позиції $i$, співпадаючих з першими символами рядка $s$.

Іншими словами, $z[i]$ - це найбільший загальний префікс рядка $s$ і її $i$-го суфікса.

**Зауваження**. В даній статті, у уникнути неопределённости, ми будемо вважати рядок 0-индексированной - тобто перший символ рядка має індекс $0$, а останній - $n-1$.
...
Ми просто для кожної позиції $i$ перебираємо відповідь для її $z[i]$, починаючи з нуля, і до тих пір, поки ми не виявимо несовпадение або не дійдемо до кінця рядка.
...
Назвемо для стислості підрядок, збігається з префіксом рядка $s$, **відрізком збіги**. Наприклад, значення шуканої Z-функції $z[i]$ - це довжелезний відрізок збіги, що починається в позиції $i$ (і заканчиваться він буде в позиції $i + z[i] - 1$).

Для цього будемо підтримувати **координати $[l;r]$ самого правого відрузку збіги**, тобто з всіх виявлених відрізків будемо зберігати той, який закінчується правіше всього. В деякому сенсі, індекс $r$ - це така межа, до якій нашо рядок вже була просканирована алгоритмом, а усе остальное - поки ще не відомо.
...
Для цього зауважимо, що підрядки $s[l \ldots r]$ і $s[0 \ldots r-l]$ **збігаються**. Це означає, що в якості початкового наближення для $z[i]$ можна взяти відповідне йому значення з відрузку $s[0 \ldots r-l]$, а саме, значення $z[i-l]$.
...
Наведемо **приклад** такий ситуації, на прикладі рядка:
...
Всё розв'язок оформлено в вигляді функції, яка по рядка повертає масив довжини $n$ - обчислену Z-функцію.
...
Доведемо, що наведений вище алгоритм працює за лінійне щодо довжини рядка час, тобто за $O(n)$.
...
У избежании путаницы, назвемо одну рядок **текстом** $t$, іншу - **образцом** $p$. Таким чином, задача полягає в тому, аби знайти всі входження зразка $p$ в текст $t$.

Для розв'язку цій задачі образуем рядок $s = p + \# + t$, тобто до зразком пріпішем текст через символ-роздільник (який не зустрічається ніде в самих рядках).

Порахуємо для отриманої рядка Z-функцію. Значить для будь-якого $i$ в відрізку $[0; length(t)-1]$ по соответствующему значенням $z[i + length(p) + 1]$ можна зрозуміти, входити або образец $p$ в текст $t$, починаючи з позиції $i$: якщо це значення Z-функції рівне $length(p)$, то да, входити, інакше - ні.
...
Отже, нехай $k$ - поточний кількість різних підрядків рядка $s$, і ми додаємо в кінець символ $c$. Очевидно, в результаті могли з'явитися деякі нові підрядки, оканчивавшиеся на цим новому символі $c$ (а саме, всі підрядки, оканчивающиеся на цим символі, але не встречавшиеся раніше).

Візьмемо рядок $t=s+c$ і інвертуємо її (запишемо символи в зворотньому порядку). Наша задача - порахувати, скільки у рядка $t$ таких префіксів, які не зустрічаються в ній більш ніде. Але якщо ми порахуємо для рядка $t$ Z-функцію і найдемо її максимальне значення $z_{\rm max}$, то, очевидно, в рядка $t$ зустрічається (не в початку) її префікс довжини $z_{\rm max}$, але не більшої довжини. Зрозуміло, префікси меншої довжини вже точно зустрічаються в ній.

Отже, ми отримали, що число нових підрядків, з'являються при дописуванні символу $c$, рівне $len - z_{\rm max}$, де $len$ - поточна довжина рядка після приписування символу $c$.

Отже, асимптотика розв'язку для рядка довжини $n$ становить $O(n^2)$.

Варто замітити, що цілковито аналогічно можна перераховувати за $O(n)$ кількість різних підрядків і при дописуванні символу в початок, а також при видаленні символу з кінця або з початку.
...
Для розв'язку порахуємо Z-функцію рядка $s$, і найдемо першу позицію $i$ таку, що $i + z[i] = n$, і при цим $n$ ділиться на $i$. Значить рядок $s$ можна стиснути до рядка довжини $i$.
