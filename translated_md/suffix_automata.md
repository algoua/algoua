...
Наприклад, з допомогою суфіксного автомата можна шукати всі входження однієї рядка в іншу, або підраховувати кількість різних підрядків даній рядка - обидві задачі він дозваляє розв'язувати за лінійне час.

На интуитивном рівні, суфіксний автомат можна розуміти як стислу інформацію про **всіх подстроках** даній рядка. Впечатляющим фактом є то, що суфіксний автомат містить всю інформацію в настільки стислому вигляді, що для рядка довжини $n$ він вимагає лише $O(n)$ пам'яті. Більш того, він можливо бути побудований також за час $O(n)$ (якщо ми припустимо розмір алфавіту $k$ константою; в іншому випадку - за час $O(n \log k)$).
...
Визначення. **Суффиксным автоматом** для даній рядка $s$ називається такий мінімальний детерминированный конечный автомат, який приймає всі суфікси рядка $s$.
...
* Одне або декілька станів позначені як **термінальні стан**. Якщо ми пройдемо з початкового стан $t_0$ по будь-якого шляхи до какого-або терминального стан, і випишемо при цим мітки всіх пройдених дуг, то вийде рядок, яка обязана бути одним з суфіксів рядка $s$.
...
Простейшим, і разом з тим важнейшим властивістю суфіксного автомата є то, що він містить в собі інформацію про всіх подстроках рядка $s$. А саме, **будь-який шлях** з початкового стан $t_0$, якщо ми випишемо мітки дуг уздовж цього шляхи, утворює обов'язково **підрядок** рядка $s$. І навпаки, будь-який підрядку рядка $s$ відповідає деякий шлях, що починається в начальном стані $t_0$.

В цілях спрощення объяснений, ми будемо говорити, що підрядку **відповідає** той шлях з початкового стан, мітки уздовж якого утворюють цю підрядок. І навпаки, ми будемо говорити, що будь-якого шляхи **відповідає** то рядок, яку утворюють мітки його дуг.
...
Розглянемо будь-яку непустую підрядок $t$ рядка $s$. Значить назвемо **безліччю закінчень** $endpos(t)$ множину всіх позицій в рядка $s$, в яких оканчиваются входження рядка $t$.

Ми будемо називати дві підрядки $t_1$ і $t_2$ $endpos$-еквівалентними, якщо їх множини закінчень збігаються: $endpos(t_1) = endpos(t_2)$. Таким чином, всі непусті підрядки рядка $s$ можна розбити на декілька **класів еквівалентності** відповідно їх безлічам $endpos$.

Виявляється, що в суфіксном автоматі **$endpos$-еквівалентним подстрокам відповідає одне і то ж стан**. Іншими словами, число станів в суфіксном автоматі рівне кількості класів $endpos$-еквівалентності серед всіх підрядків, плюс одне початкова стан. Кожному станом суфіксного автомата відповідають одна або декілька підрядків, мають одне і то ж значення $endpos$.
...
**Лема 1**. Две непусті підрядки $u$ і $w$ ($length(u) \le length(w)$) є $endpos$-еквівалентними тоді і тільки тоді, коли рядок $u$ зустрічається в рядка $s$ тільки в вигляді суфікса рядка $w$.
...
**Лема 2**. Розглянемо дві непусті підрядки $u$ і $w$ ($length(u) \le length(w)$). Значить їх множини $endpos$ або не перетинаються, або $endpos(w)$ цілком міститься в $endpos(u)$, причому це залежить від того, є $u$ суфіксом $w$ або ні:
...
Доведення. Припустимо, що множини $endpos(u)$ і $endpos(w)$ мають хоча б один загальний елемент. Значить це означає, що рядка $u$ і $w$ оканчиваются в одному і тому ж місці, тобто $u$ - суфікс $w$. Але тоді кожне входження рядка $w$ містить на своєму наприкінці входження рядка $u$, що і означає, що його множину $endpos(w)$ цілком вкладывается в множину $endpos(u)$.

**Лема 3**. Розглянемо деякий клас $endpos$-еквівалентності. Відсортуємо всі підрядки, що входять в цей клас, по невозрастанию довжини. Значить в получившейся послідовності кожний підрядок буде на одиницю коротше попередньою, і при цим бути суфіксом попередньою. Іншими словами, **підрядки, що входять в один клас еквівалентності, насправді є суфіксами один одного, і приймають всілякі різні довжини в деякому відрізку $[x;y]$**.

Доведення.

Зафіксуємо деякий клас $endpos$-еквівалентності. Якщо він містить тільки одну рядок, то коректність леми очевидна. Нехай тепер кількість стрічок більше однієї.

Згідно з лемою 1, дві різні $endpos$-эквивалентные рядка завжди такі, що одна є власним суфіксом іншої. Отже, в одному класі $endpos$-еквівалентності не можливо бути стрічок однаковою довжини.

Позначимо через $w$ довжелезну, а через $u$ - кратчайшую рядок в даному класі еквівалентності. Згідно з лемою 1, рядок $u$ є власним суфіксом рядка $w$. Розглянемо тепер будь-який суфікс рядка $w$ з довжиною в відрізку $[length(u); length(w)]$, і покажемо, що він міститься в цим ж класі еквівалентності. Насправді, цей суфікс можливо входити в $s$ тільки в вигляді суфікса рядка $w$ (оскільки більш короткий суфікс $u$ входити тільки в вигляді суфікса рядка $w$). Отже, згідно з лемою 1, цей суфікс $endpos$-еквівалентний рядка $w$, що і потрібно довести.
...
Також ми знаємо, що перші декілька суфіксів рядка $w$ (якщо ми розглядаємо суфікси в порядку зменшення їх довжини) містяться в тому ж самому класі еквівалентності, а всі інші суфікси (як мінімум, порожній суфікс) - в якихось інших классах. Позначимо через $t$ перший такий суфікс - в нього ми і проведемо суфіксну посилання.

Іншими словами, **суфіксна посилання** $link(v)$ веде в таке стан, якому відповідає **наідліннейшій суфікс** рядка $w$, находящийся в другом класі $endpos$-еквівалентності.

Тут ми припустимо, що начальному станом $t_0$ відповідає окремий клас еквівалентності (що містить тільки пусту рядок), і вважаємо $endpos(t_0) = [-1 \ldots length(s)-1]$.

**Лема 4**. Суфіксние посилання утворюють **дерево**, коренем якого є початкова стан $t_0$.

Доведення. Розглянемо довільне стан $v \ne t_0$. Суффиксная посилання $link(v)$ веде з нього в стан, якому відповідають рядка строго меншої довжини (це випливає з визначення суфіксному посилання і з леми 3). Отже, рухаючись по суфіксним посиланням, ми рано або пізно прийдемо з стан $v$ в початкова стан $t_0$, якому відповідає пустая рядок.
...
**Наведемо приклад** дерева суфіксних посилань в суфіксном автоматі, побудованому для рядка $"abcbc"$:
...
* Безліч підрядків рядка $s$ можна розбити на класи еквівалентності згідно їх безлічам закінчення $endpos$.
...
Значить всі рядка, відповідні цьому станом, є різними суфіксами рядка $longest(v)$ і мають всілякі довжини в відрізку $[minlen(v); len(v)]$.

* Для кожного стан $v \ne t_0$ визначена суфіксна посилання, ведущая в таке стан, яке відповідає суфіксу рядка $longest(v)$ довжини $minlen(v)-1$. Суфіксние посилання утворюють дерево з коренем в $t_0$, причому це дерево, по суті, є деревом отношений включения між множествами $endpos$.
...
Приступим до опису самого алгоритму. Алгоритм буде **онлайновим**, тобто буде додавати по одному символу рядка $s$, перестраивая відповідним чином поточний автомат.
...
Відповідно, вся задача тепер зводиться до тому, аби реалізувати обробку **додавання одного символу** $c$ в кінець поточній рядка. Опишемо цей процес:

* Нехай $last$ - це стан, відповідне всій поточній рядка до додавання символу $c$. (Cпершу $last = 0$, а після додавання кожного символу ми будемо змінювати значення $last$).
...
Якщо нам також потрібно знати, які вершини є **терминальными**, а які - ні, то ми можемо знайти всі термінальні вершини після побудови суфіксного автомата для всій рядка. Для цього розглянемо стан, відповідне всій рядка (воно, очевидно, у нас сохранено в змінній $last$), і будемо йти по його суфіксним посиланням, поки не дійдемо до початкового стан, і помечать кожне пройденное стан як терминальное. Легко зрозуміти, що тим самим ми пометим стан, відповідні всім суфіксам рядка $s$, що нам і потрібно.
...
Як можна побачити з опису алгоритму, суцільні і несплошные переходи призводять до різним ветвям алгоритму. Сплошные переходи називаються так тому, що, появившись вперше, вони більше ніколи не будуть змінюватися. В протилежність їм, несплошные переходи можуть змінитися при додаванні нових букв до рядка (змінитися можливо кінець дуги-переходу).

* Щоб уникнути неоднозначностей, під рядком $s$ ми будемо подразумевать рядок, для якій був побудований суфіксний автомат до додавання поточного символу $c$.

* Алгоритм починається з того, що ми створюємо нове стан $cur$, якому буде відповідати вся рядок $s + c$. Зрозуміло, чому ми зобов'язані створити нове стан - т.до. разом з додаванням нового символу виникає новий клас еквівалентності - це клас стрічок, закінчуються на добавляемом символі $c$.

* Після создания нового стан алгоритм проходится по суфіксним посиланням, починаючи з стан, відповідного всій рядка $s$, і намагається додати перехід по символу $c$ в стан $cur$. Тим самим, ми приписываем до кожному суфіксу рядка $s$ символ $c$. Але додавати нові переходи ми можемо тільки в тому випадку, якщо вони не будуть конфликтовать з вже имеющимися, тому, як тільки ми встретим вже наявний перехід по символу $c$, ми зразу ж зобов'язані зупинитися.

* Самий простий випадок - якщо ми так і дійшли до фіктивного стан $-1$, додавши всюди по новому переходу уздовж символу $c$. Це означає, що символ $c$ в рядка $s$ раніше не зустрічався. Ми успешно додали всі переходи, залишилося тільки проставить суфіксну посилання у стан $cur$ - вона, очевидно, повинна бути рівна $0$, оскільки станом $cur$ в даному випадку відповідають всі суфікси рядка $s+c$.

* Другий випадок - коли ми наткнулись на вже наявний перехід $(p,q)$. Це означає, що ми намагалися додати в автомат рядок $x+c$ (де $x$ - деякий суфікс рядка $s$, має довжину $len(p)$), а ця рядок **вже була раніше добавлена** в автомат (тобто рядок $x+c$ вже входити як підрядок в рядок $s$). Оскільки ми припускаємо, що автомат для рядка $s$ побудований правильно, то нових переходів ми більше додавати не повинні.
...
* Інший, більш складний різновид - коли перехід несплошной, тобто $len(q) > len(p) + 1$. Це означає, що станом $q$ відповідає не тільки нужная нам підрядок $w+c$ довжини $len(p) + 1$, але також і підрядки більшої довжини. Нам нічого не залишається, крім як провести **"розщеплення"** стан $q$: розбити відрізок стрічок, відповідних їй, на два підвідрізку, так що перший буде заканчиваться як раз довжиною $len(p) + 1$.
...
Остался останній крок - перенаправить деякі що входять в $q$ переходи, перенаправив їх на $clone$. Які саме що входять переходи треба перенаправить? Досить перенаправить тільки переходи, відповідні всім суфіксам рядка $w+c$, тобто нам треба продолжить рухатися по суфіксним посиланням, починаючи з вершини $p$, і до тих пір, поки ми не дійдемо до фіктивного стан $-1$ або не дійдемо до стан, перехід з якого веде в стан, отличное від $q$.
...
Залишилося оцінити сумарну асимптотику **в третьому місці** - в тому, де ми перенаправляємо переходи, провідні в $q$, на $clone$. Позначимо $v = longest(p)$. Це суфікс рядка $s$, і з кожної ітерацією його довжина зменшується - а, значить, і позиція $v$ як суфікса рядка $s$ монотонно зростає з кожної ітерацією. При цим, якщо перед першо] ітерацією циклу відповідно рядок $v$ була на глубине $k$ ($k \ge 2$) від $last$ (якщо вважати глубиной число суфіксних посилань, які треба пройти), то після останньої ітерації рядок $v+c$ стане $2$-ой суфіксному ссылкой на шляхи від $cur$ (яке стане новим значенням $last$).

Таким чином, кожна ітерація цього циклу призводить до тому, що позиція рядка $longest(link(link(last))$ як суфікса всій поточній рядка буде монотонно збільшуватися. Отже, всього цей цикл не міг отработать більш $n$ ітерацій, **що і потрібно довести**.
...
Сам суфіксний автомат будемо зберігати в вигляді масиву цих структур $state$. Як доводиться в наступному розділі, якщо $MAXN$ - це максимально возможная в програмі довжина рядка, то достатньо завести пам'ять під $2 \cdot MAXN - 1$ станів. Також ми зберігаємо змінну $last$ - стан, відповідне всій рядка на даний момент.
...
Нарешті, наведемо реалізацію основний функції - яка додає черговий символ в кінець поточній рядка, перестраивая відповідним чином автомат:
...
Число станів в суфіксном автоматі, побудованому для рядка $s$ довжини $n$, **не перевищує $2n-1$** (для $n \ge 3$).
...
При обробці цій рядка на кожної ітерації, починаючи з третьої, буде відбуватися розщеплення стан, і, тим самим, буде достигаться оцінка $2n-1$.

### Число переходів

Число переходів в суфіксном автоматі, побудованому для рядка $s$ довжини $n$, **не перевищує $3n-4$** (для $n \ge 3$).
...
Оцінимо тепер число несплошних переходів. Розглянемо кожний несплошной перехід; нехай поточний перехід - це перехід $(p,q)$ по символу $c$. Поставимо йому в відповідність рядок $u+c+w$, де рядок $u$ відповідає длиннейшему шляхи з початкового стан в $p$, а $w$ - длиннейшему шляхи з $q$ в яке-або терминальное стан. З однієї сторони, всі такі рядка $u+c+w$ для всіх несплошних переходів будуть різними (оскільки рядка $u$ і $w$ образованы тільки сплошными переходами). З іншої сторони, кожна з таких стрічок $u+c+w$, по визначенню терминального стан, буде суфіксом всій рядка $s$. Оскільки непустих суфіксів у рядка $s$ всього $n$ штук, і до тому ж вся рядок $s$ серед цих стрічок $u+c+w$ не могла міститися (т.до. всій рядка $s$ відповідає шлях з $n$ суцільних ребер), то загальне число несплошних переходів не перевершує $n-1$.
...
Відразу оговоримся, що ми припустимо, що вхідні рядок така, що кожний суфікс має собственную вершину в суфіксном дереві (оскільки для довільних стрічок це, взагалі кажучи, невірно: наприклад, для рядка $"aaa \ldots"$). Обычно цього добиваются шляхом приписування в кінець рядка какого-нибудь особого символу (зазвичай обозначаемого через знак доллара).

Для зручності введемо позначення: $\overline{s}$ - це рядок $s$, записана в зворотньому порядку, $DAWG(s)$ - це суфіксний автомат, построенный для рядка $s$, $ST(s)$ - це [суфіксне дерево](ukkonen) рядка $s$.

Введемо поняття **расширяющей посилання**: зафіксуємо вершину суфіксного дерева $v$ і символ $c$; тоді расширяющая посилання $ext[c,v]$ веде в вершину дерева, відповідну рядка $c+v$ (якщо цей шлях $c+v$ закінчується посередині ребра, то проведемо посилання в нижній кінець цього ребра); якщо такого шляхи $c+v$ взагалі ні в дереві, то расширяющая посилання не визначена. В деякому сенсі, розширюють посилання противоположны суфіксним посиланням.
...
В цілях наочності, наведемо суфіксний автомат з його деревом суфіксних посилань і відповідне суфіксне дерево для інвертованою рядка. Для прикладу візьмемо рядок $s = "abcbc"$.
...
**Лема**. Следующие три затвердження еквівалентні для будь-яких двох підрядків $u$ і $w$:

* $endpos(u) = endpos(w)$ в рядка $s$
* $firstpos(\overline{u}) = firstpos(\overline{w})$ в рядка $\overline{s}$
* $\overline{u}$ і $\overline{w}$ лежати на одному і тому ж шляхи з кореня в суфіксном дереві $ST(\overline{s})$.

Доведення її досить очевидно: якщо початку входжень двох стрічок збігаються, то одно рядок є префіксом іншої, а, значить, одно рядок лежить в суфіксном дереві на шляхи іншої рядка.
...
В термінах інвертованою рядка $\overline{s}$ це означає, що суфіксна посилання $link[x]$ веде в такий довжелезний префікс рядка, відповідної станом $x$, аби цьому префиксу соответствовало окреме стан $y$. Іншими словами, суфіксна посилання $link[x]$ веде в предка вершини $x$ в суфіксном дереві, що і потрібно довести.

**Доведення теореми 2**.

Состояния суфіксного автомата відповідають вершин суфіксного дерева.

Розглянемо довільний перехід $(x,y,c)$ в суфіксном автоматі $DAWG(s)$. Наличие цього переходу означає, що $y$ - це таке стан, клас еквівалентності якого містить підрядок $longest(x) + c$. В інвертованою рядка $\overline{s}$ це означає, що $y$ це таке стан, якому відповідає підрядок, $firstpos$ від якій (в тексті $\overline{s}$) збігається з $firstpos$ від підрядки $c + \overline{longest(x)}$.
...
Суфіксне дерево $ST(\overline{s})$ буде містити стільки ж вершин, скільки станів в $DAWG(s)$, причому вершині дерева, получившейся з стан $v$ автомата, відповідає рядок довжини $len(v)$.
...
Суфіксний автомат $DAWG(s)$ буде містити стільки ж станів, скільки вершин в $ST(\overline{s})$. У кожного стан $v$ його длиннейшая рядок $longest(v)$ буде відповідати инвертированному шляхи з кореня дерева до вершини $v$.

Згідно теоремі 2, аби побудувати всі переходи в суфіксном автоматі, нам треба знайти всі розширюють посилання $ext[c,v]$.

По-перше, зауважимо, що частина цих расширяющих посилань виходять безпосередньо з суфіксних посилань в дереві. Насправді, якщо для будь-який вершини $x$ ми розглянемо її суфіксну посилання $y = link(x)$, то це означає, що треба провести расширяющую посилання з $y$ в $x$ по першого символу рядка, відповідної вершині $x$.
...
Таким чином, описаний алгоритм за час $O(n)$ будує суфіксний автомат по суфіксному дереву для інвертованою рядка.
...
**Умова**. Дан текст $T$, і надходять запити в вигляді: дано рядок $P$, потрібно перевірити, входити або ні рядок $P$ в текст $T$ як підрядок.
...
Як тепер відповідати на один запит. Нехай поточний стан - це змінна $v$, спершу вона рівна начальному станом $t_0$. Будемо йти по символам рядка $P$, відповідним чином роблячи перехід з поточного стан $v$ в нове стан. Якщо в якийсь момент случилось, що переходу з поточного стан по нужному символу не виявилося - то відповідь на запит "ні". Якщо ж ми змогли обробити всю рядок $P$, то відповідь на запит "да".

Зрозуміло, що це буде працювати за час $O(length (P))$. Більш того, алгоритм фактично шукає довжину наидлиннейшего префікса $P$, встречающегося в тексті - і якщо вхідні зразки такі, що ці довжини маленькі, то і алгоритм буде працювати значно швидше, не обрабатывая всю рядок цілком.

### Кількість різних підрядків

**Умова**. Дано рядок $S$. Потрібно дізнатися кількість різних її підрядків.

**Асимптотика**. $O(length (S))$.

**Розв'язок**. Побудуємо суфіксний автомат по рядка $S$.

В суфіксном автоматі будь-який підрядку рядка $S$ відповідає якийсь шлях в автоматі. Оскільки повторюваних стрічок в автоматі бути не можливо, то відповідь на задачу - це **кількість різних шляхів** в автоматі, починаються в початкової вершині $t_0$.
...
Відповіддю на задачу буде значення $d[t_0]-1$ (одиниця віднімається, аби не враховувати пусту підрядок).

### Суммарная довжина різних підрядків

**Умова**. Дано рядок $S$. Потрібно дізнатися сумарну довжину всіх різних її підрядків.

**Асимптотика**. $O(length (S))$.

**Розв'язок**. Розв'язок задачі аналогічно попередньою, тільки тепер треба вважати в динаміці дві величини: кількість різних підрядків $d[v]$ і їх сумарну довжину $ans[v]$.
...
### Лексикографически k-ая підрядок

**Умова**. Дано рядок $S$. Надходять запити - числа $K_i$, і потрібно знаходити $K_i$-ую в порядку сортування підрядок рядка $S$.

**Асимптотика**. $O(length (ans) \cdot Alphabet)$ на один запит (де $ans$ - це відповідь на цей запит, $Alphabet$ - розмір алфавіту).

**Розв'язок**. Розв'язок даній задачі базируется на тією ж ідеї, що і попередні дві задачі. Лексикографически $k$-ая підрядок - це лексикографический $k$-ый шлях в суфіксном автоматі. Тому вважаючи для кожного стан кількість шляхів з нього, ми зможемо легко шукати $k$-ый шлях, рухаючись від кореня автомата.

### Найменший циклічний зсув

**Умова**. Дано рядок $S$. Потрібно знайти лексикографічно найменший її циклічний зсув.

**Асимптотика**. $O(length (S))$.

**Розв'язок**. Побудуємо суфіксний автомат для рядка $S+S$. Значить цей автомат буде містити в собі як шляхи всі циклічні зрушення рядка $S$.
...
**Умова**. Дан текст $T$, і надходять запити в вигляді: дано рядок $P$, потрібно дізнатися, скільки раз рядок $P$ входити в текст $T$ як підрядок (входження можуть перекрываться).

**Асимптотика**. Препроцесинг $O(length (T))$ і $O(length (P))$ на один запит.

**Розв'язок**. Побудуємо суфіксний автомат по тексту $T$. 

Далі нам треба зробити такий препроцесинг: для кожного стан $v$ автомата порахувати число $cnt[v]$, рівне размеру множини $endpos(v)$. Насправді, всі рядка, відповідні одному і тому ж станом, входять в $T$ однакове число раз, рівне кількості позицій в множині $endpos$.
...
Потім ми виконуємо для кожного $v$ таку операцію: $cnt[link(v)] += cnt[v]$. Смысл цього полягає в тому, що якщо рядок, відповідна станом $v$, встречалась $cnt[v]$ раз, то всі її суфікси будуть зустрічатися стільки ж.
...
**Умова**. Дан текст $T$, і надходять запити в вигляді: дано рядок $P$, потрібно дізнатися позицію початку першого входження рядка $P$.
...
**Умова**. Дан текст $T$, і надходять запити в вигляді: дано рядок $P$, потрібно вивести позиції всіх її входжень в рядок $T$ (входження могу перекрываться).
...
Нехай тепер надійшов запит - рядок $P$. Знайдемо, якого станом $t$ вона відповідає.

Зрозуміло, що $firstpos(t)$ точно має входити в відповідь. Які ще позиції треба знайти? Ми учли стан автомата, содержащее рядок $P$, однак не учли інші стан, яким відповідають такі рядка, що $P$ є їх суфіксом.
...
... після побудови автомата... for (int v = 1; v < sz; ++v) st[st[v].link].inv_link.push_back(v);
...
...
### Пошук найкоротшою рядка, не входящей в дану

**Умова**. Дано рядок $S$, і завдань определённый алфавит. Потрібно знайти таку рядок найменшою довжини, що вона не зустрічається в $S$ як підрядок.

**Асимптотика**. Розв'язок за $O(length (S))$.

**Розв'язок**. Розв'язувати буде динамическим програмування по автомату, построенному для рядка $S$.

Нехай $d[v]$ - це відповідь для вершини $v$, тобто ми вже набрали частина підрядки, оказавшись в стані $v$, і хочемо знайти найменше число символів, яке треба ще додати, аби вийти за межі автомата, знайшовши несуществующий перехід.

Вважається $d[v]$ дуже просто. Якщо з $v$ ні переходу хоча б по одному символу з алфавіту, то $d[v] = 1$: ми можемо приписать такий символ і вийти за межі автомата, отримавши тим самим шукану рядок.
...
Відповідь на задачу буде рівний $d[t_0]$, а саму рядок можна відновити, восстановив, яким чином в динаміці вийшов цей мінімум.

### Наидлиннейшая загальний підрядок двох стрічок

**Умова**. Дано дві рядка $S$ і $T$. Потрібно знайти їх найдовшу загальну підрядок, тобто таку рядок $X$, що вона є підрядком і $S$, і $T$.

**Асимптотика**. Розв'язок за $O(length(S) + length(T))$.

**Розв'язок**. Побудуємо суфіксний автомат по рядка $S$.

Будемо тепер йти по рядка $T$, і для кожного префікса шукати наідліннейшій суфікс цього префікса, встречающийся в $S$. Іншими словами, ми для кожної позиції в рядка $T$ хочемо знайти найдовшу загальну підрядок $S$ і $T$, заканчивающуюся саме в цій позиції.
...
При цим поточну довжину треба укоротить, але залишити максимально возможной. Очевидно, для цього треба присвоїти $l = len(v)$, оскільки після проходу по суфіксному посиланню нас удовлетворит підрядок будь-який довжини, відповідна цьому станом:
...
### Наибольшая загальний підрядок декількох стрічок.

**Умова**. Дано $K$ стрічок $S_i$. Потрібно знайти їх найдовшу загальну підрядок, тобто таку рядок $X$, що вона є підрядком всіх $S_i$.

**Асимптотика**. Розв'язок за $O(\sum length(S_i) \cdot K)$.

**Розв'язок**. Склеим всі рядка $S_i$ в одну рядок $T$, приписав після кожної рядка $S_i$ свій власний символ-роздільник $D_i$ (тобто введя $K$ додаткових спец. символів $D_i$):
